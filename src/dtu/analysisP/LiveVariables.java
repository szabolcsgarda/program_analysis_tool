...
public class LiveVariables extends Analysis {
    ...
    private void dealWithDeclaration(VariableDeclaration currentExpression) {
        HashSet<String> startStateRD = liveVariables.get(currentExpression.getDestinationNode());
        liveVariables.get(currentExpression.getStartNode()).addAll(startStateRD);
    }

    private void dealWithReadOperation(ReadOperation currentExpression) {
        HashSet<String> startStateRD = (HashSet<String>)liveVariables.get(currentExpression.getStartNode()).clone();
        if (currentExpression.getVariableType() == Expression.VARIABLE_VARIABLE)
        {
            //kill function
            for (Iterator<String> i = startStateRD.iterator(); i.hasNext();)
            {
                String variableName = i.next();
                if (variableName == currentExpression.getVariableName())
                    i.remove();
            ...
            startStateRD.addAll(currentExpression.getUsedVariables());
        liveVariables.get(currentExpression.getStartNode()).addAll(startStateRD);
    }
